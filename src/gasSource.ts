...
function addWorkLog(data) {
  try {
    ...
    return { success: true, message: '근무 기록이 저장되었습니다.', logId: newId };
  } catch (err) {
...
function getWeeklyWorkLogs(employeeId, weekStart) {
  try {
    ...
    logs.sort(function(a, b) {
      if (a.workDate === b.workDate) return a.startTime.localeCompare(b.startTime);
      return a.workDate.localeCompare(b.workDate);
    });
    
    return { success: true, logs: logs, weekStart: week.weekStart, weekEnd: week.weekEnd };
  } catch (err) {
...
function getWeeklySummary(employeeId, weekStart) {
  try {
    ...
    var totalMinutes = 0;
    logs.forEach(function(l) { totalMinutes += Number(l.minutesWorked) || 0; });
    ...
    return {
      ...
      settlement: settlement || { status: 'pending', actualPay: null, settledAt: null }
    };
  } catch (err) {
...
function deleteWorkLog(id, employeeId, role) {
  try {
    ...
    if (targetRowIndex === -1) return { success: false, message: '해당 근무 기록을 찾을 수 없습니다.' };
...
function getEmployeesWithWeeklySummary(weekStart) {
  try {
    ...
    return { success: true, employees: results, weekStart: week.weekStart, weekEnd: week.weekEnd };
  } catch (err) {
  ...
  try {
    ...
    for (var i = 1; i < rows.length; i++) {
      if (String(rows[i][0]).trim() === cleanId) {
        return { success: false, message: '이미 존재하는 직원 ID입니다.' };
      }
    }
    
    var createdAt = Utilities.formatDate(new Date(), TIMEZONE, 'yyyy-MM-dd HH:mm:ss');
    empSheet.appendRow([cleanId, cleanName, cleanPw, rate, true, createdAt]);
    
    return { success: true, message: cleanName + ' 직원이 추가되었습니다.' };
  } catch (err) {
    return { success: false, message: '직원 추가 중 오류: ' + err.message };
  }
}

function updateEmployee(data) {
  try {
    if (!data.employeeId) return { success: false, message: '직원 ID가 필요합니다.' };
    ...
    if (targetRowIndex === -1) return { success: false, message: '직원을 찾을 수 없습니다.' };
    
    if (data.active !== undefined) empSheet.getRange(targetRowIndex, 5).setValue(data.active);
    if (data.hourlyRate !== undefined) empSheet.getRange(targetRowIndex, 4).setValue(Number(data.hourlyRate));
